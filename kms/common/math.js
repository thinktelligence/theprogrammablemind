...
let config = {
  ...
  bridges: [
    ...
    { id: "x", isA: ['number#1'], level: 0, bridge: '{ ...next(operator) }', development: true},
    { id: "y", isA: ['number#1'], level: 0, bridge: '{ ...next(operator) }', development: true},
    { 
        where: where(),
        id: "plus", level: 0, 
        ...
        isA: ['queryable#1', 'number', 'mathematicalOperator'],
        localHierarchy: [ [['unknown', 0], ['number', 1]] ],
        levelSpecificHierarchy: [['mathematicalExpression', 1]],
        words: ['+'],
    ...
    {   
        ...
        isA: ['queryable#1', 'number#1', 'mathematicalOperator#1'],
        localHierarchy: [ [['unknown', 0], ['number', 1]] ],
        words: ['-'],
        generatorp: ({gp, context}) => `${gp(context.x)} ${context.word} ${gp(context.y)}`,
        evaluator: ({e, context}) => {
