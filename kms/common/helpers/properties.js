  ...
  createActionPrefix(args, semanticApply) {
    ...
    create.map( ({ id, isA }) => {
      if (id === operator) {
        ...
        const interpolateVerb = `{ property: "operator", verb: true }`
        const interpolate = "[" + before.map((arg) => `{ property: '${arg.tag}', subject: true }`).concat(interpolateVerb).concat(after.map((arg) => `{ property: '${arg.tag}' }`)).join(',') + "]"
